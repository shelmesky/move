...
pub(crate) fn parse_program(
    ...
    let new_targets = find_move_filenames_with_address_mapping(targets.clone())?;
    let mut deps = find_move_filenames_with_address_mapping(deps)?;
    println!("parse_program targets length {:?}, deps length {:?}", targets.clone().len(), deps.clone().len());
